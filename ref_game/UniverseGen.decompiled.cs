...
public static class UniverseGen
{
	...
	public static GalaxyData CreateGalaxy(GameDesc gameDesc)
	{
		...
		starCount = GenerateTempPoses(dotNet35Random.Next(), starCount, 4, 2.0, 2.3, 3.5, 0.18);
		...
		float num = (float)dotNet35Random.NextDouble();
		...
		for (int i = 0; i < starCount; i++)
		{
			...
			galaxyData.stars[i] = StarGen.CreateStar(galaxyData, tmp_poses[i], gameDesc, i + 1, seed, needtype, needSpectr);
		}
		...
		while (num12++ < 256)
		{
			for (int j = 0; j < tmp_drunk.Count; j++)
			{
				...
				while (num13++ < 256)
				{
					...
					VectorLF3 vectorLF2 = new VectorLF3(tmp_drunk[j].x + num14 * num17, tmp_drunk[j].y + num15 * num17, tmp_drunk[j].z + num16 * num17);
					if (!CheckCollision(tmp_poses, vectorLF2, minDist))
					{
						tmp_drunk[j] = vectorLF2;
						tmp_poses.Add(vectorLF2);
						if (tmp_poses.Count < maxCount)
						{
							break;
						}
	...
	private static void line_arragement_for_add_node(StarGraphNode node)
	{
		...
		for (int i = 0; i < node.conns.Count; i++)
		{
			...
			for (int j = i + 1; j < node.conns.Count; j++)
			{
				...
				float num4 = ((!(num > num2)) ? ((num2 > num3) ? num2 : num3) : ((num > num3) ? num : num3));
				float num5 = ((!(num < num2)) ? ((num2 < num3) ? num2 : num3) : ((num < num3) ? num : num3));
