        ...
        if (root.TryGetProperty("buildings", out var bs))
            foreach (var b in bs.EnumerateArray())
            {
                var nb = new BlueprintService.BpBuilding
                {
                    ...
                    Tilt = GetFloat(b, "tilt"), Pitch = GetFloat(b, "pitch"),
                    X2 = GetFloat(b, "x2"), Y2 = GetFloat(b, "y2"), Z2 = GetFloat(b, "z2"),
                    Yaw2 = GetFloat(b, "yaw2"), Tilt2 = GetFloat(b, "tilt2"), Pitch2 = GetFloat(b, "pitch2"),
                    OutputToSlot = (sbyte)GetInt(b, "outputToSlot"), InputFromSlot = (sbyte)GetInt(b, "inputFromSlot"),
                    OutputFromSlot = (sbyte)GetInt(b, "outputFromSlot"), InputToSlot = (sbyte)GetInt(b, "inputToSlot"),
                    OutputOffset = (sbyte)GetInt(b, "outputOffset"), InputOffset = (sbyte)GetInt(b, "inputOffset"),
