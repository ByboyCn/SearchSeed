...
### 种子查看器
- 输入 `种子号 + 恒星数量 + 资源倍率`，秒出完整星系数据（无需游戏、无需存档）；恒星数量支持 **32 ~ 64 任意值**（游戏官方范围）
...
- 支持**恒星数范围搜索**（32~64 之间任意区间，如 32~48），逐种子逐规模扫描；后台多线程异步执行，实时显示"已扫种子数 / 命中数 / 缓存命中数"，可随时停止
- 结果可直接点击种子号跳转查看器
...
| --- | --- |
| `seedId` | 种子号 |
| `starNum` | 恒星数量：32 ~ 64 的任意整数（游戏官方范围） |
...
```
POST /api/search/start      # 提交条件（可指定恒星数范围 starNumFrom~starNumTo，32~64），返回 { jobId }
